...
def test_bus_functionality():
    ...
    try:
        ...
        assert packet in host2.buffer, "Packet not received by the correct host"
        assert packet not in host1.buffer, "Packet incorrectly received by the source host"
        assert packet not in host3.buffer, "Packet incorrectly received by an unrelated host"

        packet = Packet(src="00:00:00:00:00:02", dst="00:00:00:00:00:03", payload="Test Packet")
        bus.broadcast(packet)
        open(bus.log_file, 'w').close()  # Clear log

        assert packet in host3.buffer, "Packet not received by the correct host"
        assert packet not in host1.buffer, "Packet incorrectly received by the source host"
        assert packet not in host2.buffer, "Packet incorrectly received by an unrelated host"
